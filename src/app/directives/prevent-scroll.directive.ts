...
export class IgxPreventDocumentScrollDirective
  implements AfterViewInit, OnDestroy
{
  ...
  constructor(
    @Host()
    @Optional()
    @Inject(IGX_GRID_BASE)
    private grid: IgxGridBaseDirective
  ) {}

  public ngAfterViewInit() {
    this.gridBody = this.getGridTBody();
    if (this._preventScroll) {
      this.gridBody.addEventListener('wheel', this.preventDocumentScroll, {
        passive: false,
      });
    }
  }
